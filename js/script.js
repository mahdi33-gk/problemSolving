...
//   Validate saving percentage value
  const balannce = document.getElementById('balance').innerText;
  if (savePercentage < 0) {
    alert("Provide positive saving value");
  }
  const savingAmount = (Number(savePercentage) / 100) * balannce;

  // calculate remaining balance
  const balance = (document.getElementById("balance").innerText);
  const remainingBalance = Number(balance)- savingAmount;
  console.log(remainingBalance)

  //   validate saving amount
  if(isNaN(savingAmount || isNaN(balance))){
    return alert('provide something valid.');
  }
